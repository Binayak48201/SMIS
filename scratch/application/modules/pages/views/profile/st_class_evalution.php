				...
				<div class="note note-sm note-success"><?= $evaluation->evaluation_type_name ?></div>
					<div class="col-md-12 md-radio-list">
						<?php 
							...
							$update = FALSE;
							$options = modules::run("pages/evaluation/get_evaluation_option",1,$evaluation->evaluation_type_id);
							$remarked = modules::run("pages/evaluation/get_st_evaluated",1,$evaluation->evaluation_type_id,$st_id,$exam_id,$section_id);
							if($remarked != NULL)
							{
								$update = TRUE;
	                  ...
	                  <?= $option->evaluation_option_name ?></label>
						...
						<div class="col-md-12 form-group form-md-line-input">
							<textarea class="form-control" placeholder="Remark" name='remark_<?= $evaluation->evaluation_type_id ?>' required ><?= $remarked != NULL ? $remarked->remarks : '' ?></textarea>
							<div class="form-control-focus">
							</div>
						</div>
          </div>
